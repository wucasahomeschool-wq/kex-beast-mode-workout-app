...
- **AI loading**: while the AI coach tunes the workout, a spinning comic loading ring (rotating dashed circle + pulsing dots) shows over a dimmed panel; capped so a slow/failed call still moves on. No mention of AI — label is a generic "WARMING UP…" style line (editable text).
...
- **Timer ring**: circular progress ring drains as the timer runs; last 3 seconds it pulses red and ticks bigger.
...
- **Stretch section**: gentler, slower fade-in (calmer tone) to signal cooldown.
...
- **Countdown to next tournament**: ticking pulse on the seconds.
...
**Not covered / tell me if you want it**: sound effects (deferred per your note), animated Kex character/mascot reactions, per-exercise illustrated demo animations, page-turn style transitions, haptic vibration on mobile.
...
## 4. Editor koin economy panel (only if effort remains)
An EDITOR-only settings panel with a global "koin value" multiplier slider plus one slider per koin-earning action: workout completion per difficulty level, Mommy course workout, each trophy tier (streak / workout count / difficulty / tournament win), tournament placement tiers (1st, 2nd, 3rd, top 10, rest), per-day streak bonus, and the shield price knobs (freeze base, streak scaling, rest-day discount, early-bird discount per day, discount cap). Values persist in Cloud so they apply for all users. This is last priority — if animation eats the budget, it waits for the next run.
